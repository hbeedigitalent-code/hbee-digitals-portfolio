...
const statusColors: Record<TaskStatus, string> = {
  ...
  'Waiting On Client': 'bg-purple-500/20 text-purple-500',
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        {statusColumns.map((status) => (
          <div key={status} className="rounded-xl border border-[var(--border)] bg-[var(--bg-card)] p-3">
            <div className="flex items-center justify-between mb-3">
              <span className="text-sm font-semibold text-[var(--text-primary)]">{status}</span>
              <span className="rounded-full bg-[var(--bg-section)] px-2 py-0.5 text-xs text-[var(--text-muted)]">
              ...
              {getTasksByStatus(status)
                ...
                .map((task) => (
                  <Link
                    ...
                    className={`block rounded-lg bg-[var(--bg-section)] p-3 transition hover:bg-[var(--bg-section)]/80 ${priorityColors[task.priority] || ''}`}
                  >
