      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-1">
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card-dark)] p-6">
            <h3 className="text-sm font-semibold text-white mb-4">Project Management</h3>
          ...
          <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-card-dark)] p-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="text-sm text-white">{project.client?.full_name || 'N/A'}</p>
                <p className="text-xs text-[var(--text-muted)]">{project.client?.email || ''}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-muted)]">Business</p>
                <p className="text-sm text-white">{project.client?.business_name || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-muted)]">Service</p>
                <p className="text-sm text-white">{project.service_selected || 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-muted)]">Start Date</p>
                <p className="text-sm text-white">{project.start_date ? new Date(project.start_date).toLocaleDateString() : 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-muted)]">Expected Completion</p>
                <p className="text-sm text-white">{project.expected_completion_date ? new Date(project.expected_completion_date).toLocaleDateString() : 'N/A'}</p>
              </div>
              <div>
                <p className="text-xs text-[var(--text-muted)]">Created</p>
                <p className="text-sm text-white">{new Date(project.created_at).toLocaleDateString()}</p>
              </div>
              <div className="sm:col-span-2">
                <p className="text-xs text-[var(--text-muted)]">Description</p>
                <p className="text-sm text-white">{project.description || 'No description'}</p>
              </div>
            </div>
          </div>
        </div>
