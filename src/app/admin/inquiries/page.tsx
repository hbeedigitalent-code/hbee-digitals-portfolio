...
export default function AdminInquiriesPage() {
  ...
  const [filter, setFilter] = useState<
    'all' | 'unread' | 'new' | 'contacted' | 'qualified' | 'closed'
  >('all')
  ...
  async function fetchInquiries(showLoading = true) {
  ...
  const qualifiedCount = inquiries.filter((item) => item.status === 'qualified').length
    ...
    <main className="min-h-screen text-white">
      <div className="mx-auto max-w-7xl">
        <div className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-7 text-white/55">
              View, track, and manage project inquiries submitted through your website in realtime.
            </p>
          </div>
        ...
        <div className="mb-6 grid gap-4 md:grid-cols-4">
          ...
          <StatCard label="Qualified" value={String(qualifiedCount)} />
        </div>
        ...
        {loading && (
          <div className="mb-6 grid gap-4">
            {[1, 2, 3].map((item) => (
              <div
                key={item}
                className="h-32 animate-pulse rounded-[1.6rem] border border-[#1E314A] bg-[#0E1B2D]"
              />
            ))}
          </div>
        )}
              ...
              {filteredInquiries.length === 0 ? (
                <div className="rounded-[2rem] border border-[#1E314A] bg-[#0E1B2D] p-8 text-center">
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    <div className="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl border border-[#39D97A]/20 bg-[#39D97A]/10">
                      <img
                        src="/svgs/inquiries.svg"
                        alt=""
                        className="h-7 w-7"
                        style={{
                          filter:
                    ...
                    <p className="mt-3 max-w-sm text-sm leading-7 text-white/45">
                      New project inquiries submitted through your website will appear here in realtime.
                    </p>
                  </div>
                </div>
              ) : (
