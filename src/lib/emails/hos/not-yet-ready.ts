// src/lib/emails/hos/not-yet-ready.ts
...
export async function sendNotYetReadyEmail(data: EmailData) {
  const html = `
<!DOCTYPE html>
<html>
<head>
  ...
  <title>Review Results - Hbee Digitals</title>
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .badge { display: inline-block; background: #64748b; color: #ffffff; padding: 6px 18px; border-radius: 9999px; font-size: 12px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.06em; margin-top: 12px; }
    ...
    .signoff { margin-top: 24px; padding: 20px 24px; background: #f8fafc; border-radius: 12px; border-left: 4px solid #64748b; }
    ...
    @media (prefers-color-scheme: dark) {
      ...
      .journey-line.done { background: #39D97A; }
      .signoff { background: #07111F; border-left-color: #64748b; }
...
<body>
  <div class="email-wrapper">
    <div class="email-container">
      <div class="email-header">
        ...
        <div class="badge">Review Completed</div>
      </div>
      <div class="email-content">
        <h1>Your Growth Review Results</h1>
        <p>Hi <strong>${data.firstName}</strong>,</p>
        <p>Thank you again for completing the Hbee Digitals Growth Readiness Assessment.</p>
        <p>We've now completed our review of your business.</p>
        <p>Based on our current evaluation, your business isn't yet ready for our <strong>Q3 Growth Support Implementation Initiative</strong>.</p>
        <p><strong>This decision doesn't reflect the quality or potential of your business.</strong></p>
        <p>Rather, it indicates that there are foundational areas we'd recommend strengthening before implementation is likely to have the greatest impact.</p>
        <p>Your Growth Profile outlines the observations from our review and highlights the areas we'd encourage you to focus on before requesting another assessment in the future.</p>
        <div class="journey-tracker">
          ...
          <div class="journey-steps">
            ...
            <span class="journey-line done"></span>
            <span class="journey-step done"><span class="icon">✓</span> Profile</span>
            <span class="journey-line"></span>
            <span class="journey-step"><span class="icon">○</span> Future Reassessment</span>
          </div>
        </div>
        <p style="text-align: center; margin: 24px 0;">
          <a href="${data.profileUrl}" class="btn-secondary">View Your Growth Profile</a>
        </p>
        <p>We appreciate the opportunity to review your business and wish you continued success as you grow.</p>
        <div class="signoff">
  ...
  try {
    await resend.emails.send({
      ...
      to: data.email,
      subject: 'Your Growth Review Results',
      html,
    })
    console.log('✅ Not yet ready email sent to:', data.email)
  } catch (error) {
    console.error('Error sending not yet ready email:', error)
  }
}